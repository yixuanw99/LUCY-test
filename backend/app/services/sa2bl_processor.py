...
def sa2bl_from_csv(beta_table_file_name: str, epidish_file_name: str) -> pd.DataFrame:
    ...
    # Load necessary data
    beta_table_path = PROCESSED_BETA_TABLE_DIR / beta_table_file_name
...
if __name__ == "__main__":
    ...
    # methylation_data = pd.read_csv('backend/data/processed_beta_table/our_all_samples_normed_processed.csv', index_col='probeID')
    # result_from_pd = sa2bl_from_pd(methylation_data, run_epidish_with_csv("backend/data/processed_beta_table/our_all_samples_normed_processed.csv"))
    
    # # 下面這行從頭跑，需要跑很久，所以還沒有測試
    # result_from_pd = sa2bl_from_pd(champ_df_postprocess(process_idat("D:/SideProject/EpiAging/SVD_test/raw/Sample_Sheet.csv", "D:/SideProject/EpiAging/SVD_test/raw")), run_epidish_with_csv("data/processed_beta_table/our_all_samples_normed_processed.csv"))
